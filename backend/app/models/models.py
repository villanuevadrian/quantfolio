import uuid
from sqlalchemy import String, Numeric, Date, ForeignKey, CheckConstraint
...
class Portfolio(Base):
    ...
    owner: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.user_id"),nullable= False)
    
    __table_args__ = (
        CheckConstraint("asset_type IN ('Stock', 'Fund', 'ETF', 'Crypto')", name = "ck_portfolio_asset_type"),
    )
...
class Transaction(Base):
    ...
    asset: Mapped[str] = mapped_column(String(50), ForeignKey("stock.asset_code"), nullable= False)
    ...
    portfolio: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid= True), ForeignKey("portfolio.portfolio_id"), nullable= False)
     
    __table_args__ = (
        CheckConstraint("operation_type IN ('Buy', 'Sell')", name= "ck_transaction_operation_type"),
    )
...
class Event(Base):
    ...
    asset: Mapped[str | None] = mapped_column(String(50), ForeignKey("stock.asset_code"),nullable= True)
    owner: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.user_id"), nullable= False)